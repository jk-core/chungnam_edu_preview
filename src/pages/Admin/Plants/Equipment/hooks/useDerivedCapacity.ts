 ...
 *
 * 서버는 `instCapa` 를 값으로 받으므로 폼이 그 값을 들고 있어야 한다 — 자동으로 채우되 현장
...
export function useDerivedCapacity(methods: UseFormReturn<EquipmentFormValues>, modules: ModuleProduct[]) {
  ...
  const [series1, parallel1, series2, parallel2, moduleProductId] = useWatch({
    control,
    name: ['series1', 'parallel1', 'series2', 'parallel2', 'moduleProductId'],
  });

  const wattPerPanel = modules.find((item) => item.id === moduleProductId)?.wattPerPanel;
